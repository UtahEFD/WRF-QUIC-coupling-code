...
% Relief
if ~isempty(SimData.Matrix)
    for x = 1:size(SimData.Matrix,2)
        for y = 1:size(SimData.Matrix,1)
            if SimData.Matrix(y,x) > 0.5                
...
% Vegetation
for a = 1:size(VegData.Coord,2) %%% for each vegetation zone
    for b = 1:size(VegData.Coord{a},1) %%% for each points in this zone
        x = VegData.Coord{a}(b,1);
        y = VegData.Coord{a}(b,2);
        if SimData.Matrix == 0
            BaseHeight = 0;
        else
            BaseHeight = SimData.Matrix(y,x);
        end
        fprintf(fid,'!Start Building %i\n', ind);
        fprintf(fid,'%g\t\t\t!Group ID\n', 1);
        ...
        fprintf(fid,'\n');
        fprintf(fid, '%.3f\t\t\t%s\n', VegData.Coeff{a}, '!Attenuation Coefficient');
        fprintf(fid,'%g\t\t\t!Height [m]\n', VegData.Height{a});
        fprintf(fid,'%g\t\t\t!Base Height (Zfo) [m]\n', BaseHeight);
        fprintf(fid,'%g\t\t\t!Centroid X [m]\n', (x-1/2)*SimData.dx);
        fprintf(fid,'%g\t\t\t!Centroid Y [m]\n', (y-1/2)*SimData.dy);
...
for n = 1:size(BdData.List,1)
    ...
    if GeomNbType~=6 % Not a polygon
        ...
        BaseHeight = SimData.Matrix(CenterY,CenterX); %%% Bd base elevation is defined by its center elevation.
