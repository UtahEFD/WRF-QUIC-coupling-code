function Z0 = RoughnessLengthFunction(SimData,Z0DataSource,Z0FLAG)

%%% In case 1 INPUT file must be WRF RESTART file
%%% In case 2 INPUT file must be WRF OUTPUT
%%% In case 3 INPUT var should be Z0 constant value on the domain

%%% Computes a roughness length matrix covering each points of the inner grid

switch Z0FLAG
    case 1   %%% WRF RESTART file
        Z0 = ncread(Z0DataSource,'Z0');
        Z0 = Z0(SimData.XSTART:SimData.XEND, SimData.YSTART:SimData.YEND);
    case 2   %%% WRF OUTPUT file
        LU = ncread(Z0DataSource,'LU_INDEX');
        LU = LU(SimData.XSTART:SimData.XEND, SimData.YSTART:SimData.YEND,1); 
        Z0 = zeros(size(LU,1),size(LU,2));
        for i = 1:size(LU,1)
            for j = 1:size(LU,2)
                switch LU(i,j) %%% USGS LAND COVER CATEGORIES
...
%%%%%%%%%%%%%%%%%%%%%%%%%      MODIS-WINTER      %%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    case 3   %%% User-defined constant
        Z0 = repmat(Z0DataSource, SimData.nx, SimData.ny);
end